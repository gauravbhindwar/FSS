...
import { FiSearch, FiCheckCircle, FiXCircle, FiX, FiPrinter } from "react-icons/fi";
...
const UserManagementDashboard = () => {
  ...
  const getReportPrint = async (selectedUser) => {
    try {
      ...
      link.download = `${selectedUser.name}.html`;
      link.click();
  ...
  const usersWithoutFormFilled = filteredUsers.filter((user) => !user.isFormFilled);
      ...
      <div className="bg-white rounded-lg shadow overflow-hidden">
        ...
        <div className="block xl:hidden">
          {usersWithFormFilled.map((user) => (
            <div key={user._id} className="p-4 m-2 bg-white rounded-lg shadow-md hover:shadow-lg hover:bg-gray-50 transition-all duration-150 ease-in-out">
        ...
        <div className="hidden xl:block">
          <table className="w-full text-sm sm:text-base">
            <thead className="bg-gray-50">
              <tr>
                <th className="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">MUJid</th>
            ...
            <tbody>
              {usersWithFormFilled.map((user) => (
                <tr key={user._id} className="hover:bg-gray-50 transition-colors duration-150 ease-in-out">
                  <td className="px-4 py-2 whitespace-nowrap">{user.mujid}</td>
        ...
        <div className="relative w-full max-w-md p-4 sm:p-6 mx-auto bg-white rounded-lg shadow-xl">
          ...
          <div className="flex items-center mb-4 sm:mb-6">
            <div className="w-12 h-12 sm:w-16 sm:h-16 mr-3 sm:mr-4 rounded-full bg-gray-200 flex items-center justify-center text-xl sm:text-2xl font-bold text-gray-800">
              {selectedUser?.Name
                ? selectedUser.Name.split(" ").map((word) => word[0].toUpperCase()).join("")
                : "UN"}
            </div>
            <div>
          ...
          <div className="p-2 sm:p-4 mb-4 sm:mb-6 bg-gradient-to-r from-purple-400 via-pink-500 to-red-500 rounded-lg">
            <p className="text-white text-sm sm:text-base">mujId: {selectedUser.mujid}</p>
          </div>
                ...
                {Object.entries(selectedUser.allSelectedCourses).map(
                  ([semester, courses], idx) => (
                    <motion.div
                      ...
                      whileTap={{ scale: 0.98 }}
                    >
                      <h4 className="text-sm sm:text-base font-semibold text-gray-700">Semester: {semester}</h4>
